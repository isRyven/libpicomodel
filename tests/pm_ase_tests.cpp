...
TEST_CASE("Should load simple ase") {
    picoModel_t *model = PicoLoadModel("../tests/assets/model.ase", 0);
    REQUIRE(model != NULL);
    PicoFreeModel(model);
}

TEST_CASE("Should correctly parse simple ase") {
    picoModel_t *model = PicoLoadModel("../tests/assets/model.ase", 0);
    ...
    picoVec2_t sts[] = { { 0.f, 1.f },{ 0.f, 0.f },{ 1.f, 1.f },{ 0.f, 0.f }, { 1.f, 0.f } };
